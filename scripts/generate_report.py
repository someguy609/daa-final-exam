...
RESULTS = [
    ('S1', '36',    '2', '6', '15%', 'A*',       8,    4167,  106,   31),
    ('S1', '36',    '2', '6', '15%', 'Dijkstra',  5,    2115,  106,   24),
    ('S1', '36',    '2', '6', '15%', 'GBFS',      3,    434,   290,   68),
    ('S2', '144',   '2', '6', '15%', 'A*',        1,    510,   144,   25),
    ('S2', '144',   '2', '6', '15%', 'Dijkstra',  3,    1179,  144,   25),
    ('S2', '144',   '2', '6', '15%', 'GBFS',      0,    56,    166,   24),
    ('S3', '400',   '2', '6', '15%', 'A*',        6,    6158,  254,   60),
    ('S3', '400',   '2', '6', '15%', 'Dijkstra', 23,   16859,  254,   61),
    ('S3', '400',   '2', '6', '15%', 'GBFS',      1,    111,   312,   54),
    ('S4', '1,024', '2', '6', '15%', 'A*',       33,   25313,  290,   73),
    ('S4', '1,024', '2', '6', '15%', 'Dijkstra', 52,   45715,  290,   73),
    ('S4', '1,024', '2', '6', '15%', 'GBFS',   4165,   97843, 2253,  667),
    ('S5', '2,025', '2', '6', '15%', 'A*',       53,   35541,  535,  117),
    ('S5', '2,025', '2', '6', '15%', 'Dijkstra',172,   89549,  535,  115),
    ('S5', '2,025', '2', '6', '15%', 'GBFS',   2306,   36877, 9102, 2113),
]
...
bullet('Nodes expanded: ',
       'A* expands significantly fewer states through heuristic guidance. '
       'In S4 (1,024 vertices), A* expands 25,313 nodes vs. Dijkstra\'s 45,715, '
       'a 1.8x reduction.')
bullet('Runtime: ',
       'A* is consistently faster on large grids. '
       'In S5 (2,025 vertices), A* finishes in 53 ms vs. Dijkstra\'s 172 ms, a 3.2x speedup.')
bullet('Regime preference: ',
       'A* dominates on large grids where the heuristic provides strong guidance. '
       'Dijkstra is marginally preferable on trivially small grids '
       'where heuristic computation overhead is non-negligible relative to total search time.')
bullet('GBFS vs optimal: ',
       'GBFS produces fewer per-path node expansions on small grids but its suboptimal '
       'routes cause dramatically more CBS conflicts. In S4, GBFS took 4,165 ms '
       'against A*\'s 33 ms, a 126x runtime penalty, confirming that greedy '
       'low-level search is counterproductive in multi-agent settings.')
...
body(
    ...
    '(106, 144, 254, 290, 535). This empirically confirms that both low-level '
    'implementations are correct and that the CBS engine applies constraints '
    'and computes path costs consistently. GBFS returns strictly higher SOC in '
    'all cases (290, 166, 312, 2,253, 9,102), confirming the theoretical '
    'prediction that greedy search is suboptimal.'
)
body(
    'Makespan values differ slightly between A* and Dijkstra at equal SOC '
    '(e.g., S1: A* makespan 31 vs. Dijkstra 24). This is expected: both '
...
body(
    'For A*, wall-clock runtime grows from 8 ms at S1 (36 vertices) to 53 ms at S5 '
    '(2,025 vertices), a 6.6x increase for a 56x increase in grid size. '
    'Sub-linear scaling confirms that CBS conflict resolution remains manageable '
    'with 2 robots and 6 items, as the constraint tree stays shallow.'
)
body(
    'For GBFS the conflict explosion is dramatic at S4: 4,165 ms vs. A*\'s 33 ms. '
    'The GBFS makespan at S4 (667 steps) vs. A*\'s (73 steps) reveals extreme '
    'suboptimality: robots take roughly 9x longer paths that cross the grid '
    'repeatedly and generate many pairwise conflicts.'
)
body(
...
body(
    ...
    'practical, solving 45x45 grids with 2 robots and 6 items in 53 ms.'
)
